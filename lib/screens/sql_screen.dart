...
class _SqlScreenState extends State<SqlScreen> {
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        if (audioHandler == null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Audio service not initialized. Check logs for details.',
              ),
  ...
  Future<void> _cleanDuplicates(BuildContext context) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Duplicates cleaned!')));
      setState(() {
        _overviewFuture = _loadOverview();
      });
    } catch (e) {
              ...
              ListView.builder(
                ...
                itemBuilder: (context, songIndex) {
                  ...
                  return ListTile(
                    ...
                    trailing: Text(
                      _formatDuration(
                        Duration(milliseconds: (song['duration'] as int?) ?? 0),
                      ),
